...
// 		std::cout << i << " ";
		for(unsigned int j = 0; j < currentColumns; j++) {
			if(maze[i][j])
				...
				if(i == currentPositionRow && j == currentPositionColumn)
					std::cout << "0";
				else
					std::cout << " ";
		}
...
void SmartRobotActionPlugin::Environment::addWall(unsigned int row, unsigned int column) {
	maze[row][column] = true;
	std::cerr << "Added " << row << " " << column << std::endl;;
}


void SmartRobotActionPlugin::Environment::setDimension(unsigned int rows, unsigned int columns) {
	if(rows < 3 || columns < 3) {
...
void SmartRobotActionPlugin::Environment::moveUp() {
	if(currentPositionRow == 0 || maze[currentPositionRow - 1][currentPositionColumn]) {
...
void SmartRobotActionPlugin::Environment::moveDown() {
	if(currentPositionRow == currentRows - 1 || maze[currentPositionRow + 1][currentPositionColumn]) {
...
void SmartRobotActionPlugin::Environment::moveLeft() {
	if(currentPositionColumn == 0 || maze[currentPositionRow][currentPositionColumn - 1]) {
...
void SmartRobotActionPlugin::Environment::moveRight() {
	if(currentPositionColumn == currentColumns - 1 || maze[currentPositionRow][currentPositionColumn + 1]) {
...
void SmartRobotActionPlugin::Environment::setCurrentPosition(unsigned int row, unsigned int column) {
	if(maze[row][column]) {
...
void SmartRobotActionPlugin::SmartRobotActionAtomWeights::retrieve(
	...
	// call Term::Term with second argument true to get a quoted string!
	//
	Term termUp(ID::MAINKIND_TERM | ID::SUBKIND_TERM_CONSTANT, std::string(concatstreamWup.str()));
	out.push_back(registry.storeTerm(termUp));
	Term termDown(ID::MAINKIND_TERM | ID::SUBKIND_TERM_CONSTANT, std::string(concatstreamWdown.str()));
	out.push_back(registry.storeTerm(termDown));
	Term termLeft(ID::MAINKIND_TERM | ID::SUBKIND_TERM_CONSTANT, std::string(concatstreamWleft.str()));
	out.push_back(registry.storeTerm(termLeft));
	Term termRight(ID::MAINKIND_TERM | ID::SUBKIND_TERM_CONSTANT, std::string(concatstreamWright.str()));
...
void SmartRobotActionPlugin::SmartRobotAction::execute(Environment& environment,
  ...
  Registry& registry = *pregistry;
	...
	if (registry.getTermStringByID(parms[0]) == "move") {
		if (registry.getTermStringByID(parms[1]) == "up")
			...
			if (registry.getTermStringByID(parms[1]) == "down")
				...
				if (registry.getTermStringByID(parms[1]) == "left")
					...
					if (registry.getTermStringByID(parms[1]) == "right")
		...
		if(registry.getTermStringByID(parms[0]) == "addWall") {
			...
			if(registry.getTermStringByID(parms[0]) == "setDimension")
				...
				if(registry.getTermStringByID(parms[0]) == "setPosition")
					environment.setCurrentPosition(parms[1].address, parms[2].address);
				else
					if(registry.getTermStringByID(parms[0]) == "finishedFirstExecution")
...
std::vector<PluginAtomPtr> SmartRobotActionPlugin::createAtoms(
	...
	ret.push_back(
		PluginAtomPtr(new SmartRobotActionAtomFirstExecution, PluginPtrDeleter<PluginAtom>()));
	ret.push_back(
		PluginAtomPtr(new SmartRobotActionAtomGetPosition, PluginPtrDeleter<PluginAtom>()));
	ret.push_back(
		PluginAtomPtr(new SmartRobotActionAtomPreviousMovement, PluginPtrDeleter<PluginAtom>()));
	ret.push_back(
		PluginAtomPtr(new SmartRobotActionAtomWeights, PluginPtrDeleter<PluginAtom>()));
	return ret;
}
