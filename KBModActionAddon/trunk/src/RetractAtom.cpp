...
namespace dlvhex
{
  namespace kbmod
  {
	...
	RetractAtom::RetractAtom(KBModActionAddon& kbmod) : kbmod_addon(kbmod)
	{
		//addInputConstant();	
		//addInputConstant();			
	}
    ...
    RetractAtom::execute(const Tuple & parms) throw(PluginError)
    {
	  ...
	  {	
		...
		if(parms[i].isInt() || parms[i].isSymbol() || parms[i].isVariable())
		{
		...
		{	
			std::string ruleStr_par=parms[i].getString();
			 if(ruleStr_par[0]!='\"' || ruleStr_par[ruleStr_par.size()-1]!='\"')
		 	 {	
		  		throw PluginError("The parameters of the atom #retract should be a quoted string or an integer!");
			  }
